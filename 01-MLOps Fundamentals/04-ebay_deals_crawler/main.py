...
for deal in deals:
    try:
        ...
        # Find the tag "span" with the attribute itemprop="price"
        product_price = deal.find_all("span", {"itemprop": "price"})[0].get_text().replace("$", "").replace(" VND", "")
        products.append({"product_name": product_name, "product_price": product_price, "img_url": img_url})
    except:
